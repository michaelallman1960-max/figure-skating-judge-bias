...
DB_PATH = "/Users/allman/Library/CloudStorage/Dropbox/Dropbox Mike/Judging Bias/figure_skating_ijs_seed.sqlite"
...
def compute_counterfactual_tss(conn, entry_id, exclude_judge_id, deductions,
    ...
    if not elem_scores: return None
    tes = 0.0
...
def create_tables(conn):
    ...
    for tname in ["lojo_scores", "lojo_event_summary", "integrity_checks"]:
...
def run_lojo_for_event(conn, event_id, cur_write):
...
def main():
    ...
    for i, (event_id, comp_name, discipline, segment) in enumerate(events):
        summaries = run_lojo_for_event(conn, event_id, cur_write)
        ...
        if (i + 1) % 10 == 0 or i == 0:
            print(f"  [{i+1}/{len(events)}] {comp_name} | {discipline} {segment} — {len(summaries)} judges, winner_changes={wc}")
        conn.commit()
