...
class SinglesPairsParser:
    ...
    # Pattern to split run-together numbers like "10.009.50" → ["10.00", "9.50"]
    _RUNON_FLOAT = re.compile(r'(\d+\.\d+)(?=\d)')

    def _split_pcs_tokens(self, line: str) -> List[str]:
        """Split a PCS line into tokens, handling run-together floats."""
        # Insert spaces between run-together floats (e.g. "10.009.50")
...
def insert_performance(conn: sqlite3.Connection,
    ...
    # Insert elements — actual schema: element_no, element_code, base_value, panel_goe_points
    # (panel_element_score is optional; no 'info' column in elements table)
    for elem in perf.elements:
        ...
        """, (entry_id, elem.element_no, elem.element_code,
              elem.base_value, elem.panel_goe, elem.scores_panel))
...
COMPETITIONS = {
    ...
    # 'owg2018': {...},
    'milano2026': {
        # OWG2026: milano2026 dir has Men SP/FS + Pairs SP/FS
        # (Women's events and Pairs FS may not be in DB if not yet competed)
